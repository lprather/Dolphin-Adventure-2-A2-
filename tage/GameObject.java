...
public class GameObject {
  ...
  private Matrix4f oldRotation, newRotation, rotAroundAvatar;
  ...
  public ObjShape getShape() {
    return shape;
  }

  /** assigns an ObjShape to this GameObject */
  public void setShape(ObjShape sh) {
    shape = sh;
  }
  ...
  public void setIsTerrain(boolean t) {
    isTerrain = t;
  }
  ...
  protected void setParentToNull() {
    parent = null;
  }
  ...
  protected void addChild(GameObject g) {
    children.add(g);
  }

  protected void removeChild(GameObject g) {
    children.remove(g);
  }
  ...
  public void lookAt(float x, float y, float z) {
    ...
    if (fwd.equals(0, 1, 0)) right = new Vector3f(1f, 0f, 0f); else right =
      (new Vector3f(copyFwd.cross(0f, 1f, 0f))).normalize();
  ...
  public void setLocalLocation(Vector3f location) {
    localTranslation.setTranslation(location);
    update();
  }
  ...
  public float getHeight(float x, float z) {
    ...
    return (
      ...
      Engine
        .getEngine()
        .getRenderSystem()
        .getHeightAt(heightMap.getTexture(), x, z)
    );
  }

  /** moves game object forward/back a set amount for the elapsed time.
   * input: integer indication of forward or backward desired movement, time, keyValue.
   * if improper int value for fwd/back given, it is set based on keyValue.
  */
  public void fwd(int fwd, float time, float keyValue, boolean inBounds) {
    if (fwd != 1 && fwd != 0) {
      if (keyValue < -.2) {
        fwd = 0;
      } else {
  ...
  /** turns game object right/left a set amount for the elapsed time.
  ...
  public void yaw(int right, float time, float keyValue) {
    ...
    if (right == 1) {
      rotAroundAvatar =
        new Matrix4f().rotation(0.002f * time, new Vector3f(0f, 1f, 0f));
    } else if (right == 0) {
      rotAroundAvatar =
        new Matrix4f().rotation(-0.002f * time, new Vector3f(0f, 1f, 0f));
    } else if (keyValue < -.2) {
    ...
    newRotation.mul(rotAroundAvatar);
  ...
  */
  public void pitch(boolean up, float time, float keyValue) {
    ...
    if (up || keyValue < -.2) {
      rotAroundAvatar =
        new Matrix4f().rotation(-.03f * time, new Vector3f(1f, 0f, 0f));
    } else {
      rotAroundAvatar =
        new Matrix4f().rotation(.03f * time, new Vector3f(1f, 0f, 0f));
    }
